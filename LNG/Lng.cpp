...
void ad6::tree::_pre_par( void )
{
  ...
  for (size_t i = 0; i < buf_size; i++)
  {
    ...
    if (isdigit(buf[i]))
    {
      toks[toks_size++] = token(_getNum(&i), str_counter + 1, i + 1);
      continue;
    }
    ...
    if (isalpha(buf[i]))
    {
      ...
      toks[toks_size++] = token(TOK_STR, 0, str_counter + 1, i + 1, buf + i, end_word);
      i += end_word - 1;
      continue;
    }
    toks[toks_size++] = token(TOK_SMB, 0, str_counter + 1, i + 1, buf + i, 1);
  }
